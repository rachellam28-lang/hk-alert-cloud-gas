...
- Added `trend_matrix.html` as a primary navigation page after `板塊輪動`. It is an in-system automated table, not an Excel download. The visible matrix now keeps day and night on the same row: `60D max-cycle range + 20D big-cycle range + 5D mid-cycle range + 1D small-cycle range + date + day High / Low / Mid / Open / Close + night High / Low / Mid / Open / Close`. Mobile scroll stays inside the table; no day/night tab switch is used.
...
- Row colours are an independent disclosed rule, not the Facebook author's private formula: the latest three Trend scores are averaged; `>= +1.5` is blue `吸納／偏多`, `<= -1.5` is red `派發／偏空`, otherwise charcoal `過渡／等確認`. A gold divider closes every three consecutive phase sessions. Yellow cells are prior-20-session extremes; green cells are phase-change midpoints.
- Price rhythm is calculated at three horizons from real observed bars: Daily = one-session high-low range, Weekly = rolling five-session max high minus min low, Monthly = rolling 20-session max high minus min low. Each reports current range against the median and upper quartile of the latest 60 valid samples. On the 2026-07-17 HSI snapshot this verified as current `834 / 1,319 / 2,703` points versus medians `337 / 1,044 / 2,120`; these values update with data and are not hard-coded targets.
- Every date displays its weekday. Wednesday and Thursday receive an explicit `反轉觀察窗` label per user observation, but this is never an automatic reversal signal; price-midpoint structure, cycle expansion and completed night direction still have to confirm.
...
- On 2026-07-18, rebuilt `options_levels.html` as a split option-chain ladder: CALL on the left, strike in the middle, PUT on the right. The default HSI view shows prior/current volume, official settlement, estimated premium notional, current OI, OI change, side OI share, and proportional OI bars. Every leaf column is sortable and mobile scrolling stays inside the table.
- Per the user's navigation decision, `板塊輪動` is a primary navigation item immediately after `Market`; it is removed from the More panel so there is only one route entry.
- `scripts/build_options_levels.py` now parses the latest two Big5 HKEX HSI option daily reports (`hsiocYYMMDD.htm`). Official total volume, settlement, IV, open interest and OI change are preserved per month/strike/side; prior OI is exactly current OI minus official OI change. Estimated premium notional is clearly labelled and equals settlement × total volume × HKD 50. Missing values remain unavailable and are never replaced with zero.
- Provider order is Futu OpenD when its US quote backend and option snapshots are genuinely healthy, then MarketData.app's documented option-chain API. `MARKETDATA_API_TOKEN` or `MARKETDATA_TOKEN` expands the default SPY/QQQ/IWM/GLD/NVDA/AAPL/TSLA universe; without a token, only MarketData's explicitly unlocked AAPL chain can refresh. The current Longbridge account has valid stock LV1 but no OPRA OpenAPI entitlement, so Longbridge option quote returned `301604 no quote access` and is not presented as a working options source.
- HSI is now the default and does not depend on OPRA. The 2026-07-17 official report produced three active contract months with 100% OI coverage; US symbols retain the existing Futu/MarketData fallback and may remain unavailable without entitlement. Daily refresh is best-effort and preserves the previous observed snapshot as stale on a total source failure.
- Direct Cloudflare allowlist, rebuild pipeline, publish bundle, repo audit, navigation, guide and sorting regression coverage include the page and data file.
...
- Direct Wrangler deployment: `https://996b61bb.hk-alert-cloud-gas.pages.dev`. Production `1733` daily dual view rendered 1,040 observed candles in total, showed compact `姤`, full `小暑/姤` hover text and `小暑／姤` signal card, and passed all `58/58` tests without JavaScript errors or mobile overflow.
