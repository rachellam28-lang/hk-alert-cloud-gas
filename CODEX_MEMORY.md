...
- Added `scripts/build_options_levels.py` and compact `data/options_levels.json`. Call/Put walls use the strike with the highest observed side OI. Max Pain minimizes aggregate intrinsic payout across listed strikes and is disabled below 80% OI field coverage. Missing values remain unavailable; they are never replaced with zero.
...
- The current published snapshot is honestly PARTIAL: AAPL only, refreshed 2026-07-18 from observations through 2026-07-16, with three expiries and 100% observed OI coverage. Daily refresh is best-effort and preserves the previous observed snapshot as stale on a total source failure.
