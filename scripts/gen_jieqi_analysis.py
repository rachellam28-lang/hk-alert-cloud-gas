...
<div class="wrap">
  <section class="hero">
    <div>
      ...
      <div class="note" style="margin-top:10px;color:#b7cdf1;font-size:12px"><b>新版節氣表</b>：第一屏會先見到 24 節氣日期表，再睇統計表。</div>
    </div>
    <div class="hero-meta">
...
function renderCycleRail() {
  ...
  if (!terms.length) {
    ...
    if (!fallback.length) {
      host.innerHTML = '<tr><td colspan="4" class="note">暫時冇 calendar data。</td></tr>';
      return;
    }
    host.innerHTML = fallback.map((r, idx) => `
      <tr>
        <td>${idx + 1}</td>
  ...
  host.innerHTML = terms.map((t, idx) => `
    <tr>
      <td>${idx + 1}</td>
      <td>${esc(t.name)}</td>
      <td>${esc(t.date)}</td>
      <td>${t.date.slice(5, 7)}月</td>
    </tr>`).join('');
}
