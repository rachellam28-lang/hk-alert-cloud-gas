...
function filter(cat) {{
  ...
  let rows;
  if (cat === 'all') rows = DATA;
  else if (['🔥 大股東著數','⚡ 有機會','➖ 中性','❄️ 不利'].includes(cat)) rows = DATA.filter(d => (d.analysis||{{}}).verdict === cat);
  else rows = DATA.filter(d => d.category === cat);
  render(rows);
}}
...
function sortTable(col) {{
  ...
  sortCol = col;
  let rows = currentFilter === 'all' ? [...DATA] : DATA.filter(d => {{
    if (['🔥 大股東著數','⚡ 有機會','➖ 中性','❄️ 不利'].includes(currentFilter)) return (d.analysis||{{}}).verdict === currentFilter;
