...
> 港股 HOLDINGS（中央結算系統）持倉追蹤 Dashboard  
> 最後更新：2026-05-30
...
追蹤港股 HOLDINGS 持倉數據 — 睇邊個大戶/券商正在收貨或散貨。
...
| 來源 | 說明 |
|------|------|
| **HOLDINGS 持倉** | HKEX 每日 HOLDINGS 持股報告（持倉%、Top5/Top10 集中度、HHI） |
| **股價** | GAS 實時報價 + Futu/Longbridge 歷史數據（今日Δ%、52週高低、PE、Beta） |
...
1. **HOLDINGS% ↑ + 股價 ↑** → 大戶收貨中，跟勢追入
2. **HOLDINGS% ↑ + 股價 ↓** → 大戶壓價收貨，留意見底信號
3. **HOLDINGS% ↓ + 股價 ↑** → 大戶散貨中，高位減持
...
| 現價 (lp) | GAS優先 → 本地價格快取 fallback，sortable |
| 今年Δ%🔺 (yo_pct) | (lp-yo)/yo×100，🔺=lp>yo，sortable |
| HOLDINGS% (tp) | HOLDINGS 總持倉% |
| Top5% (t5) | Top5 集中度 |
| 5日Δ (d5) | 5日 HOLDINGS% 變化，🟢增持 🔴減持 |
| 🔥/❄️ (su/sd) | 連續增持/減持日數 |
| 今日Δ% (chg) | 今日股價變幅，sortable |
...
比較兩個交易日 HOLDINGS 持倉，detect >10萬股 participant-level 變動。

**📥 存入（綠色）：** 券商 + 增持股數 → 大戶存入 HOLDINGS  
**📤 提出（紅色）：** 券商 + 減持股數 → 大戶提取 HOLDINGS

- 總量 = 📥 + 📤 總和
- 總值 = 總量 × 現價（億/萬格式）
- 佔比 = 轉倉量 / HOLDINGS 總持股 × 100%
...
| 2 | Pct Consistency | holdings_daily.total_pct vs SUM(holdings.pct) | 誤差 ≤5% |
| 3 | Shares Consistency | holdings_daily.total_shares vs SUM(holdings.shares) | 誤差 ≤2% |
...
| 數據 | 頻率 |
|------|------|
| HOLDINGS 數據 | 每日 HKEX update 後（約 7am HKT） |
| 股價 + 停牌 | 每日 9pm HKT refresh |
| 信號 | GAS 自動推送 |
...
```
holdings-debug/
├── holdings/              核心 DB + scripts
│   ├── holdings.db         SQLite (local, 不入 git)
...
├── holdings.html           Dashboard 源碼
├── data/                GitHub Pages 數據: holdings.json / transfers.json / market.json
├── signals.html         港股訊號
├── watchlist.html       ⭐自選
└── history.html         每日訊號歷史
```
...
5. regenerate_json.py → refresh holdings.json

6. detect_transfers.py → 轉倉 → transfers.json

7. cp holdings.html index.html && git push → deploy
```
...
# Backfill
cd holdings && python -m scripts.backfill --start 2026-05-29

# Fill missing
cd holdings && python scripts/fill_missing.py 2026-05-29

# Regenerate JSON
cd holdings && python scripts/regenerate_json.py

# Detect transfers
cd holdings && python scripts/detect_transfers.py

# Compute metrics
cd holdings && python -m scripts.compute_metrics

# Deploy
cp holdings.html index.html && git add . && git commit -m "deploy" && git push
```
...
| DROP/DELETE holdings.db | 數據永久丟失 | FATAL-002: backup first |
| NaN/Infinity in JSON | Dashboard 白畫面 | sanitize before export |
| 5-digit code | API error | drop leading zero |
| HTML edit with write_file | Line number leak corrupt | 只用 patch tool |
| Wide format holdings.json | Dashboard 白畫面 | Narrow format only |
